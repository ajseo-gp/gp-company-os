...
- 가격, 마진, 납기, 계약, 결제와 법률·규정 판단. 단, 대표가 승인한 활성
  `Commerce Control Rules` 범위 안의 가격·할인·프로모션 실행은 `DEC-0013`과
  `SOP-014`를 따른다.
- 레시피 확정, 생산 가능성, 시험결과·기능성·사용기한 확정
- 고객 또는 외부 대상 발송·게시. 단, 채널별 첫 승인과 `DEC-0013`·`SOP-014` Gate를
  통과한 상품 등록·콘텐츠·가격·프로모션 외부 쓰기는 승인된 범위에서 실행할 수 있다.
- 데이터 삭제·덮어쓰기와 비가역적 변경. 활성 Commerce Control Rules 안에서 변경 전
  snapshot과 rollback이 검증된 가역적 채널 상태 변경은 `DEC-0013`을 따른다.
- ACTIVE Decision과 충돌하는 변경
- 권한·책임자·근거 문서가 불명확한 고위험 실행

즉시 응답이 필요하지 않은 규칙 밖 판단과 정책 충돌은 `DEC-0015`에 따라
`DECISION_QUEUE`에 보존하고, 영향받지 않는 승인 범위의 작업을 계속한다. hard stop은
해당 실행과 부작용을 격리하되 서로 독립적인 작업까지 임의로 중단하지 않는다.
