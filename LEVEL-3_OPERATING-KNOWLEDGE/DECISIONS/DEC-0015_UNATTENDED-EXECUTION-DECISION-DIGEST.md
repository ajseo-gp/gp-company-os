...
10. 대표 또는 Codex의 OS 인계문이 원격에 존재하는 정확한 40자리 `OS-Ref`를 특정하여
    적용·재개를 지시하면, 해당 SHA는 그 Task의 승인된 고정 기준이다. PR의 merge 여부는
    배포·공식 기준선 상태이지 같은 SHA의 Task 적용 승인을 다시 묻는 근거가 아니다.
    Worker는 지정된 SHA의 원격 존재, 인계문과 검증 Evidence를 확인한 뒤 진행하며,
    `main` 미포함을 이유로 적용 여부를 `DECISION_QUEUE`에 다시 넣거나 대표에게 재승인받지
    않는다. 인계문에 이미 답이 있는 항목도 질문으로 다시 만들지 않는다.
...
6. 명시 지정된 `OS-Ref`를 `main` 미포함 사유로 재질문하거나, 인계문에 이미 답이 있는
   항목을 `DECISION_QUEUE`에 중복 등록한 건수를 측정하며 성공 기준은 0건이다.
