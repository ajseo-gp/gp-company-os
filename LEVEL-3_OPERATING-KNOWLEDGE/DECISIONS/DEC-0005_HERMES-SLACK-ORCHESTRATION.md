...
3. 모든 Hermes 작업 요청은 `Task-ID`와 40자리 commit SHA 형식의 `OS-Ref`를 포함한다.
4. Hermes는 요청의 `OS-Ref`에 고정된 `ajseo-gp/gp-company-os` 문서만 `gh api`로
...
| GP Workbench·GitHub | 작업 상태, 실행 로그, Issue·Branch·PR 증거 관리 | Slack 승인 의도 추정 |
| GPmarketing bridge | GPmarketing의 `bot-relay`, `macmini` 채널 polling과 별도 relay | GPcompany Hermes 작업 수신·응답 |

## 통제 원칙

- `OS-Ref`가 없거나 유효하지 않으면 실행하지 않고 `[BLOCKED]`로 보고한다.
