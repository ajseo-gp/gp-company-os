...
4. 외부 발송·공개, 계약·법률·규정 판단, 정책 예외, 비가역 변경은 명시적 승인 전
   실행하지 않는다. 단, `DEC-0013`의 쿠팡 신규 상품 등록은 채널별 첫 1건 승인 후
   허용 필드·표현·SKU·rollback·기록 Gate를 모두 통과한 `L1` 범위에서 Claude Code가
   실행할 수 있다. `L2` 기존 상품 변경은 건별 승인 없이는 실행하지 않는다.
...
- `DEC-0013` 허용 범위를 벗어난 외부 상품 등록·수정·게시
