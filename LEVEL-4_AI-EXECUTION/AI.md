...
- 실행 전에 관련 ACTIVE Knowledge와 비교 가능한 EXPERIMENT·FAILURE를 읽고, 적용·비적용
  이유 또는 `NO_APPLICABLE_KNOWLEDGE` 검색 범위를 남긴다.
...
- 반복 Task는 Outcome·학습 후보·Enforcement Point를 남기고 다음 비교 실행에서 재사용을
  검증한다. Knowledge 생성만으로 학습 완료로 보고하지 않는다.
