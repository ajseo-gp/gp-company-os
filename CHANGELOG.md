...
- `SOP-016`을 ACTIVE 전환해 자연어 정책 접수, 문서 유형 분류, 영향 검토, 정확한 revision 승인,
  OS main 효력 발생, 권한 부여·회수와 하위 저장소 적용·검증 절차를 정의
- 대표가 작성하지 않는 GPcompany OS 변경 내부 Envelope를 추가해 정책 상태와 Brand·Factory·
  Hub·Agent 적용 상태를 분리 기록
- `WF-009`를 ACTIVE 전환해 대표 자연어 입력부터 OS 정본, Hermes·Hub 전달, Brand·Factory·Agent
